...
class Stop0l_trigger(Module):
    ...
    def get_efficiency(self, trigger_name, kinematic):
        ...
        xrange = np.array([eff.GetX()[i] - eff.GetErrorXlow(i) for i in range(eff.GetN()) ])
        xrange = np.append(xrange, 99999)
        findbix = np.searchsorted(xrange, kinematic)-1
        if findbix == -1:
            return 0, 0, 0
        return eff.GetY()[findbix], eff.GetErrorYlow(findbix) , eff.GetErrorYhigh(findbix)
